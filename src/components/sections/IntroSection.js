const IntroSection = () => {
  return (
    <article className=" mx-[2rem] notebook:mx-[15rem] desktop:mx-[22rem] full:mx-[30rem] relative top-[8rem] notebook:top-[10rem] desktop:top-[12rem] full:top-[20rem] mb-[5rem] animate-opacityChangeY">
      ...
      <div className="absolute  top-[0.4rem]  desktop:top-[0.75rem]  font-extrabold text-[2.4rem] notebook:text-[4.62rem] desktop:text-[4.5rem] full:text-[8.2rem]  text-[#84A98C]">
        Martin Labato
      </div>

      <div className="absolute  top-[3rem]  notebook:top-[4.5rem] desktop:top-[5.5rem] full:top-[8rem] font-extrabold text-[1.85rem] notebook:text-[3.4rem] desktop:text-[3.3rem] full:text-[6.1rem]  text-[#354F52]">
        I build experiences
      </div>
